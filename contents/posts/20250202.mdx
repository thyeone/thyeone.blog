...
**Bundle Analyzer**를 통해 번들 사이즈를 분석해봤습니다.

<img src="/posts/20250202_4.png" />
<br />
<img src="/posts/20250202_5.png" />

아이콘 컴포넌트 50개가 모두 트리 쉐이킹이 되지 않은 상태로 포함되어 있었으며 **Parsed size**가 약 **30KB**로 측정되었습니다.
그래서 `React.lazy()`를 통해 컴포넌트를 동적으로 가져오는 방식으로 해결할 수 있지 않을까? 라는 생각을 했습니다.
...
<img src="/posts/20250202_6.png" />

트리 쉐이킹에는 성공하고 아이콘도 정상적으로 로드되지만, 아이콘이 약간의 지연 후에 렌더링되고 있었습니다. Layout Shift 현상이 없도록 Fallback UI를 넣어보았으나 지연은 짧지만 어색할 정도로 분명히 체감이 되는 수준입니다.
그리고 아이콘 하나당 정말 커야 **1KB**밖에 안되는 수준을 고려하면 번들 사이즈를 감소시키는데 큰 의미가 없다고 생각합니다.

그래서, 첫 웹사이트 로드 시 **번들에 모든 아이콘을 로드**시키는 첫 번째 방법을 채택하게 되었습니다.
